"""
Run TCN style transfer on pre-selected distinctive pairs.

Loads pairs from distinctive_pairs.json and performs style transfer.
"""
...
def optimize_tcn_style_transfer(
    ...
    for step in range(num_steps):
        ...
        if verbose and (step % 50 == 0 or step == num_steps - 1):
            print(f"  Step {step:3d}/{num_steps}: distance = {distance:.4f}, best = {best_distance:.4f}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run TCN style transfer on distinctive pairs')
    ...
    parser.add_argument('--pairs_file', type=str, default='distinctive_pairs.json')
    parser.add_argument('--output_dir', type=str, default='distinctive_pairs_style_transfer')
    ...
    print("="*80)
    print("TCN Style Transfer on Distinctive Pairs")
    print("="*80)
    ...
    # Load distinctive pairs
    print("Loading distinctive pairs...")
    ...
    for pair in tqdm(pairs, desc="Processing pairs"):
        ...
        idx_input = pair['index_a']
        idx_target = pair['index_b']
        name_input = pair['track_a']
        name_target = pair['track_b']
        initial_distance = pair['distance']

        print(f"\n{'='*80}")
        print(f"Pair {pair_id}/{len(pairs)}")
        print(f"{'='*80}")
        print(f"Input:  {name_input}")
        print(f"Target: {name_target}")
        print(f"Initial distance: {initial_distance:.4f}")

        # Load tracks
        stems_input, mixture_input, _, _ = dataset[idx_input]
        stems_target, mixture_target, _, _ = dataset[idx_target]
        ...
        # Save audio files
        input_path = audio_dir / f"pair{pair_id:02d}_input_{name_input.replace(' ', '_').replace('/', '-')}.wav"
        target_path = audio_dir / f"pair{pair_id:02d}_target_{name_target.replace(' ', '_').replace('/', '-')}.wav"
        output_path = audio_dir / f"pair{pair_id:02d}_output_{name_input.replace(' ', '_').replace('/', '-')}_to_{name_target.replace(' ', '_').replace('/', '-')}.wav"
